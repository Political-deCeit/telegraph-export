...
OFFTOPIC_ATT = [
    ...
    "accordion",
    "el-editorial-source",
    "video__end-slate__tertiary-title",
    "adblocker",
    "tagline",
    "navmenu",
    "topHeader",
    ...
    "socialBTN",
    "page-view",
    "pn-single-post-wrapper__social",
    "pn-pagicle-widget",
    "mashsb-container",
    "give-form-wrap",
...
def _decompseAds(soup):
    ...
    for item in soup.find_all("p"):
        if matchKey(item.text, P_AD_WORDS):
            _decompose(item)
        if "align=" in str(item) and "right" in str(item) and item.text:
            _decompose(item)


def _decomposeOfftopic(soup, url, args={}):
    for item in list(soup.find_all())[1:]:
        if _isOffTopic(item.attrs) or item.name in OFFTOPIC_TAG:
            _decompose(item)
